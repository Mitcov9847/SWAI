# Лабораторная работа №4. Расширенные возможности Zabbix: графики, отчёты, зависимые и вычисляемые элементы
...
Элемент dependent (зависимый) не получает данные напрямую от агента — он использует значение основного (master) элемента и может применять к нему дополнительную обработку.
Элемент calculated (вычисляемый) не имеет собственного источника данных и формирует значение по заданному выражению, используя другие элементы.

**2. Как сопоставлять значения нескольких метрик на одном графике?
Для сравнения нескольких метрик создаются комбинированные графики, в которых добавляют несколько элементов данных с различными подписями и цветами. Это позволяет визуально оценивать взаимосвязь показателей, например загрузки CPU и использования оперативной памяти.

** 3. Почему стоит использовать шаблоны вместо ручной настройки хостов?
Использование шаблонов обеспечивает единообразие конфигурации для большого числа устройств. Все элементы данных, триггеры и графики задаются один раз и применяются к разным хостам, что ускоряет процесс настройки и уменьшает вероятность ошибок.

** 4. Как влияет интервал обновления данных на производительность системы?
Малый интервал обновления повышает точность данных, но увеличивает нагрузку на сервер и сеть.
Большой интервал снижает актуальность информации, но делает работу системы быстрее и стабильнее.
Поэтому важно выбирать интервал, учитывая баланс между нагрузкой и своевременностью данных.

---

## Вывод

В ходе выполнения лабораторной работы были изучены расширенные возможности мониторинга и визуализации данных в системе Zabbix. Был разработан собственный шаблон, включающий основные элементы данных, триггеры и вычисляемые показатели, который был успешно назначен хосту. Это обеспечило сбор ключевых метрик: загрузки процессора, состояния оперативной памяти, сетевой активности и дискового пространства.

Использование зависимых и вычисляемых элементов позволило повысить гибкость анализа без увеличения нагрузки на систему. На основе собранных данных был создан дашборд с графиками, таблицами и сводками событий, обеспечивающий наглядный контроль состояния инфраструктуры. Настроенные триггеры и уведомления гарантируют оперативное информирование при превышении критических порогов.

Таким образом, цели лабораторной работы достигнуты: изучены возможности Zabbix по созданию шаблонов, визуализации метрик и настройке событий, что подтверждает готовность системы к применению в реальных условиях мониторинга.
